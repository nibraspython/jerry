...
from jerry.plugins.helpers import call_back_in_filter
...
@Client.on_message(filters.command("cban") & filters.group)
def bam(_, message):
    try:
...
@Client.on_message(filters.command("cunban") & filters.group)
def bam(_, message):
    user = message.reply_to_message
    admeme = bot.get_users(message.from_user.id)
    if admeme.status == "creator" or "administrator" and user.sender_chat:
